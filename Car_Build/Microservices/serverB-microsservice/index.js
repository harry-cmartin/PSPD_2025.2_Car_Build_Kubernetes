...
const calcularPrecoTotal = (itens) => {
  ...
  const chassiCount = itens.filter(item => 
    item.peca.nome.toLowerCase().includes('chassi')
...
const pricingService = {
  Calcular: (call, callback) => {
    try {
      ...
      console.log(`[SERVER B] Calculando preço para ${itens.length} tipos de itens`);
      
      itens.forEach((item, index) => {
        console.log(`[SERVER B] Item ${index + 1}: ${item.peca.nome} (ID: ${item.peca.id}) - Qtd: ${item.quantidade} - Valor unitário: R$ ${item.peca.valor}`);
      });
      ...
      console.log(`[SERVER B] Preço calculado - Subtotal: R$ ${precoTotal.toFixed(2)}`);
...
server.bindAsync(serverAddress, grpc.ServerCredentials.createInsecure(), (err, port) => {
  ...
  console.log(`[SERVER B] Microserviço B (Pricing) rodando em ${serverAddress}`);
  server.start();
});