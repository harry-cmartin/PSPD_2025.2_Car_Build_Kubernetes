                            ...
                            <div className="items-purchased">
                                ...
                                {(purchaseResult.itensComprados || purchaseResult.itens_comprados || []).map((item, index) => (
                                    <div key={index} className="purchased-item">
                                        ...
                                        <span className="item-price">R$ {(Number(item.peca.valor) * Number(item.quantidade)).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                ))}
                            </div>
                        ...
                        <ul className="cart-items-list">
                            {itensSelecionados.map((item) => (
                                <li key={item.id} className="cart-item">
                                    ...
                                    <strong>R$ {(item.valor * item.quantidade).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong>
                                </li>
                            ))}
                        </ul>

                        <div className="cart-summary">
                            <p>Subtotal: <strong>R$ {pricing?.preco?.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong></p>
                            <p>Frete: <strong>R$ {pricing?.frete?.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong></p>
                            <p className="cart-total">💰 Total: <strong>R$ {total.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</strong></p>
                        </div>
